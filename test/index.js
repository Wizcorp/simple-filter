...
describe('simple-filter', function() {
    ...
    it('can add indexes', function() {
        filter.addIndex('name', function(value) {
            return value.name;
        });
        filter.addIndex('forks', function(value) {
            return value.forks;
        });
        filter.addIndex('stars', function(value) {
            return value.stargazers_count;
        });
        filter.addIndex('updated', function(value) {
            return (new Date(value.updated_at)).getTime();
        });
        filter.addIndex('fork', function(value) {
            return value.fork;
        });
    ...
    it('can do a comparison using a function', function() {
        ...
        assert.equal(2, filter.get({
            updated: morningUpdate
        }).length);
    });
